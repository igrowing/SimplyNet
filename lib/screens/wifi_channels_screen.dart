...
class _WifiChannelsScreenState extends State<WifiChannelsScreen>
  ...
  static int _freqToChannel(int f) {
    ...
    if (f >= 5170 && f <= 5825) return (f - 5170) ~/ 5 + 34;
    return 0;
  }
  ...
  List<_WifiNetwork> _demo() => [
    ...
    _WifiNetwork(ssid: 'HomeNet5G',    bssid: 'AA:BB:CC:DD:EE:05', rssi: -50, channel: 36, band: '5'),
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_scanning)
            ...
            IconButton(icon: const Icon(Icons.refresh), tooltip: 'Re-scan', onPressed: _scan),
        ],
        bottom: TabBar(
          controller: _tabs,
...
class _ChannelChart extends StatelessWidget {
  ...
  final String             band;
  const _ChannelChart({required this.networks, required this.channels, required this.band});
  ...
  Widget build(BuildContext context) {
    ...
    final colorMap = {for (var i = 0; i < ssids.length; i++) ssids[i]: palette[i % palette.length]};
    ...
    return OrientationBuilder(builder: (ctx, orientation) {
      ...
      if (isLandscape) {
        return Row(children: [
          ...
          SizedBox(width: MediaQuery.of(context).size.width * 0.4, child: list),
        ]);
      } else {
        return Column(children: [
          Expanded(child: chart),
          const Divider(height: 1),
          SizedBox(height: MediaQuery.of(context).size.height * 0.35, child: list),
        ]);
      }
    });
  }
    ...
    return ListView(
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(children: [
            const Expanded(child: Text('SSID',   style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold))),
            SizedBox(width: 48, child: Text('Ch',   style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.right)),
            SizedBox(width: 64, child: Text('RSSI', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.right)),
            SizedBox(width: 64, child: Text('Quality', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.right)),
          ]),
        ),
        ...
        ...nets.map((n) => Padding(
          ...
          child: Row(children: [
            Container(width: 10, height: 10, margin: const EdgeInsets.only(right: 6),
                decoration: BoxDecoration(color: colorMap[n.ssid],
                    borderRadius: BorderRadius.circular(2))),
            Expanded(child: Text(n.ssid,
                style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis)),
            SizedBox(width: 48, child: Text('ch ${n.channel}',
                style: const TextStyle(fontSize: 11), textAlign: TextAlign.right)),
            SizedBox(width: 64, child: Text('${n.rssi} dBm',
                style: const TextStyle(fontSize: 11, fontFamily: 'monospace'),
                textAlign: TextAlign.right)),
            SizedBox(width: 64, child: Text(_quality(n.rssi),
...
class _ChannelPainter extends CustomPainter {
  ...
  const _ChannelPainter({required this.byChannel, required this.channels, required this.colorMap});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gridP = Paint()..color = Colors.grey.withValues(alpha: 0.2)..strokeWidth = 0.5;
    ...
    for (final level in [-40, -60, -80]) {
      ...
      tp..text = TextSpan(text: '${level}dB',
          style: const TextStyle(color: Colors.grey, fontSize: 8))..layout();
    ...
    for (var ci = 0; ci < channels.length; ci++) {
      ...
      tp..text = TextSpan(text: '$ch',
          style: const TextStyle(fontSize: 9, color: Colors.grey))..layout();
...
class _WifiNetwork {
  ...
  const _WifiNetwork({required this.ssid, required this.bssid,
